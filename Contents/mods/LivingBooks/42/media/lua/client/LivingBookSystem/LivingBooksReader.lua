...
ISHotbar.onKeyKeepPressed = function(key)
    ...
    if hotbar and not hotbar.keyPressedMS then
        -- Vanilla bug: keyPressedMS can be nil here (e.g. digit keys
        -- typed into a mod text field never cleanly reach
        -- onKeyStartPressed), causing a crash on
        -- getTimestampMs() - self.keyPressedMS. Seed it so the
